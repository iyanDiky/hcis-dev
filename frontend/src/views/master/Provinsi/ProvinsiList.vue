<template>
  <div class="row">
    <div class="col-12">
      <div class="card">
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center mb-4">
            <h5 class="card-title fw-semibold mb-0">Data Provinsi</h5>
          ...
          <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <div>
              <input type="text" v-model="params.search" @input="onSearchInput" class="form-control form-control-sm" placeholder="Cari provinsi...">
            </div>
          </div>
      ...
      <!-- Modal Form -->
      <div class="modal fade" id="provinsiModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="provinsiModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="provinsiModalLabel">{{ isEdit ? 'Edit' : 'Tambah' }} Provinsi</h5>
            ...
            <form @submit.prevent="submitForm">
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control" v-model="formData.provinsi" required>
                </div>
              </div>
...
const fetchData = async () => {
  try {
    loading.value = true
    // Sesuai requirement, backend POST
    const response = await api.post('/provinsi/list', {
    ...
    // Asumsi standard Laravel paginate response format
...
const closeForm = () => {
  ...
  setTimeout(() => {
    formData.value = { id: null, provinsi: '' }
...
const submitForm = async () => {
  try {
    ...
    if (isEdit.value) {
      await api.post('/provinsi/update', {
        id: formData.value.id,
        provinsi: formData.value.provinsi
      })
    } else {
      await api.post('/provinsi/create', {
        provinsi: formData.value.provinsi
      })
    }
    closeForm()
...
const confirmDelete = async (item) => {
  const result = await Swal.fire({
    ...
    html: `Apakah Anda yakin ingin menghapus provinsi <strong>${item.provinsi}</strong>?`,
    icon: 'warning',
...
const executeDelete = async (id) => {
  try {
    await api.post('/provinsi/delete', { id })
...
onMounted(() => {
  if (window.bootstrap) {
    formModal = new window.bootstrap.Modal(document.getElementById('provinsiModal'))
  }
  fetchData()
})
</script>
